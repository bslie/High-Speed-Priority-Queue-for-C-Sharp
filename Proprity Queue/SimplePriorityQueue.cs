...
public class SimplePriorityQueue<TItem, TPriority> : IPriorityQueue<TItem, TPriority>
{
    ...
    public void UpdatePriority(TItem? item, TPriority priority)
    {
        lock (_queue)
        {
            var updateMe = GetExistingNode(item) ?? throw new InvalidOperationException(
    ...
    public IEnumerator<TItem> GetEnumerator()
    {
        ...
        lock (_queue)
        {
            ...
            queueData.AddRange(_queue.Select(node => node.Data));
        }
    ...
    private void RemoveFromNodeCache(SimpleNode? node)
    {
        ...
        if (!_itemToNodesCache.TryGetValue(node.Data, out var nodes)) return;
        nodes.Remove(node);
    ...
    public bool IsValidQueue()
    {
        lock (_queue)
        {
            ...
            if (_itemToNodesCache.Values.SelectMany(nodes => nodes).Any(node => !_queue.Contains(node))) return false;

            // Check all items in queue are in cache
            if (_queue.Any(node => GetExistingNode(node.Data) == null)) return false;
